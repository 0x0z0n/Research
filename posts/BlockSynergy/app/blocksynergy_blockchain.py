...
class Blockchain:
    ...
    def proof_of_work(self, data):
        ...
        while not check:
            ...
            if pow_hash[:len(self.difficulty)] == self.difficulty:
                check = True
    ...
    def validation(self, chain):
        for i in range(1, len(chain)):
            ...
            if previous_hash != real_previous_hash or block_hash != real_block_hash or block_hash[:len(self.difficulty)] != self.difficulty:
    ...
    def validate_block(self, block):
        ...
        if len(txns) > 5 or previous_hash != real_previous_hash or block_hash != real_block_hash or block_hash[:len(self.difficulty)] != self.difficulty:
            print("Block Invalid!")
            return False
        else: 
            print("Block valid!")
    ...
    def sync(self):
        for node in self.nodes:
            ...
            try:
                ...
                print("Error reaching Node")
                continue

            self.newchain = data

            if self.validation(self.newchain) and len(data) > len(self.chain):
    ...
    def simulate_txn(self):
        if len(self.chain) < 3 and len(self.pending_transactions) == 5 and not self.pending_rewards:
        ...
        if len(self.pending_transactions) < 20:
            sender_priv = SigningKey.generate(curve=NIST256p)
            sender = sender_priv.verifying_key.to_string().hex()

            receiver_priv = SigningKey.generate(curve=NIST256p)
            receiver = receiver_priv.verifying_key.to_string().hex()

            amount = randint(1, 100)
            timestamp = str(datetime.now())
            txn_string = f"{timestamp}{sender}{receiver}{amount}"
            txn_hash = hashlib.sha256(txn_string.encode()).digest()
            signature = sender_priv.sign(txn_hash)
            transaction = {
                'timestamp': timestamp, 
                'sender': sender,
                'receiver': receiver,
            ...
            if not self.pending_rewards and len(self.chain) == 2:
                transaction2 = {
                    'timestamp': timestamp, 
                    'sender': "Blockchain_Reward",
                    'receiver': receiver,
...
class Wallet:
    ...
    def save_wallet(self, filename):
        data = {
            'private_key': self.priv_key.to_string().hex(),
    ...
    def calc_balance(self, blockchain, pending_transactions):
        for block in blockchain:
            if block['index'] == 1:
                continue
            ...
            for txn in block['data']:
                ...
                elif self.address == txn['receiver']:
                    self.balance += txn['amount']

        for pend_txn in pending_transactions:
            if self.address == pend_txn['sender']:
                self.balance -= pend_txn['amount']
            elif self.address == pend_txn['receiver']:
                self.balance += pend_txn['amount']
