...
cmd = (
    "curl -s -c /tmp/hc.jar http://127.0.0.1:5000/dashboard >/dev/null; "
    "curl -s -b /tmp/hc.jar -c /tmp/hc.jar "
    "-F action=upload_contract -F contract_file=@/tmp/hank_contract.json "
    "http://127.0.0.1:5000/dashboard -o /tmp/hu.html; "
    "grep -o 'Contract loaded' /tmp/hu.html; "
    "curl -s -b /tmp/hc.jar -c /tmp/hc.jar "
    "-d action=contract_mint -d contract_mint_amount=1 "
    "http://127.0.0.1:5000/dashboard -o /tmp/hm.html; "
    "echo '---MINT RESPONSE---'; cat /tmp/hm.html | head -30"
)
print(execute_as_walter(cmd))